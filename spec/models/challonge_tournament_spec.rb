  ...
  context 'helpers' do
    describe '.slug_from_url' do
      it 'with perfect URL' do
        expect(described_class.slug_from_url('https://challonge.com/foo')).to eq('foo')
      end

      it 'with perfect URL with locale' do
        expect(described_class.slug_from_url('https://challonge.com/fr/foo')).to eq('foo')
      end
      ...
      it 'with URL too long' do
        expect(described_class.slug_from_url('https://challonge.com/fr/foo/ranks')).to eq('foo')
      end

      it 'with only slug' do
        expect(described_class.slug_from_url('foo')).to eq('foo')
      end

      it 'with nil' do
        expect(described_class.slug_from_url(nil)).to eq(nil)
      end

      it 'with empty URL' do
        expect(described_class.slug_from_url('')).to eq(nil)
      end

      it 'with bad URL' do
        expect(described_class.slug_from_url('https://challonge.com/fr/')).to eq(nil)
      end
      ...
      it 'with bad host' do
        expect(described_class.slug_from_url('https://goo.gl/foo')).to eq(nil)
      end

      it 'with challenging URL' do
        expect(described_class.slug_from_url('https://challonge.com/fr/ism6hmen')).to eq('ism6hmen')
      end
    end
  end
